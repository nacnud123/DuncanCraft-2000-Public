...
namespace VoxelGame.Utils
{
    public static class WorldPositionHelper
    {
        ...
        public static ChunkPos WorldToChunkPos(Vector3 worldPos) => new ChunkPos(
                (int)Math.Floor(worldPos.X / Constants.CHUNK_SIZE),
                (int)Math.Floor(worldPos.Z / Constants.CHUNK_SIZE)
            );
        ...
        public static bool IsInChunkBounds(Vector3i localPos) => localPos.X >= 0 && localPos.X < Constants.CHUNK_SIZE && localPos.Y >= 0 && localPos.Y < Constants.CHUNK_HEIGHT && localPos.Z >= 0 && localPos.Z < Constants.CHUNK_SIZE;

        public static bool IsOnChunkBoundary(Vector3i localPos) => localPos.X == 0 || localPos.X == Constants.CHUNK_SIZE - 1 || localPos.Z == 0 || localPos.Z == Constants.CHUNK_SIZE - 1;
    }
}